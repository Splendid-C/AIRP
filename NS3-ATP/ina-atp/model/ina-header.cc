...
InaHeader::GetSerializedSize() const{
    ...
    return sizeof(m_param) + sizeof(m_workerId) + sizeof(m_epochId) + sizeof(m_jobId) 
    + sizeof(m_psId) + sizeof(m_workerNum) + sizeof(m_packetType) + sizeof(m_edgeSwitchIdentifier);
}
...
void
InaHeader::SetParam(uint32_t param){
...
void
InaHeader::SetPSId(uint32_t psId){
    NS_LOG_FUNCTION(this << psId);
    m_psId = psId;
}

uint32_t
InaHeader::GetPSId() const{
    NS_LOG_FUNCTION(this);
    return m_psId;
}
...
void
InaHeader::SetEdgeSwitchIdentifier(uint32_t edgeSwitchIdentifier){
    NS_LOG_FUNCTION(this << edgeSwitchIdentifier);
    m_edgeSwitchIdentifier = edgeSwitchIdentifier;
}

uint32_t 
InaHeader::GetEdgeSwitchIdentifier() const{
    NS_LOG_FUNCTION(this);
    return m_edgeSwitchIdentifier;
}
