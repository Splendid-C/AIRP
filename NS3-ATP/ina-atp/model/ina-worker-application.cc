  	...
  	static TypeId tid = 
		TypeId ("ns3::InaWorker")
			...
			.AddAttribute("PSAddress", "The PS address of the application",
						AddressValue (),
						MakeAddressAccessor (&InaWorker::m_psAddress),
						MakeAddressChecker ())
...
InaWorker::StartApplication(){
    ...
    if (!m_socket6)
    {
        ...
        Inet6SocketAddress local6 = Inet6SocketAddress(Ipv6Address::GetAny(), 9999);
        if (m_socket6->Bind() == -1)
        {
            NS_FATAL_ERROR("Failed to bind socket");
        }
    ...
    m_socket6->SetRecvCallback(MakeCallback(&InaWorker::HandleRead, this));
...
InaWorker::GeneratePacket(uint32_t sequence, uint32_t param){
	...
	inaH.SetEdgeSwitchIdentifier(0);
