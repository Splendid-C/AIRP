...
class InaAggregatorHelper
{
    public:
        ...
        void AddJob(uint32_t jobId, uint32_t subWorkerNum, uint32_t head, uint32_t tail,map<uint32_t, ns3::Address> workerAddr,vector<uint32_t> workerIdList);
...
class InaWorkerHelper
{
    public:
        ...
        void SetAttribute(std::string name, const AttributeValue &value);
        // void AssignPendingJobList(vector<JobList> pendingJobList);
        ApplicationContainer Install(NodeContainer c);
