...
InaWorker::GetTypeId (){
  	static TypeId tid = 
		TypeId ("ns3::InaWorker")
			...
			.AddAttribute("TotalSubJobNum", "The subjob num of the bigjob",
						UintegerValue(0),
						MakeUintegerAccessor(&InaWorker::m_subJobNum),
						MakeUintegerChecker<uint32_t>())
			.AddAttribute("TotalJobCount", "The totaljob count of the bigjob",
						UintegerValue(0),
						MakeUintegerAccessor(&InaWorker::m_totalJobCount),
						MakeUintegerChecker<uint32_t>())
			;
  return tid;
}
...
InaWorker::SetupSocket(){
	...
	if (!m_socket)
    {
        TypeId tid = TypeId::LookupByName("ns3::UdpSocketFactory");
        m_socket = Socket::CreateSocket(GetNode(), tid);
        InetSocketAddress local = InetSocketAddress(Ipv4Address::GetAny(), 99);
...
InaWorker::Send(){
	...
	InetSocketAddress remoteAddress = InetSocketAddress(Ipv4Address::ConvertFrom(m_peerAddress), m_peerPort);
	 
	if(m_socket->SendTo(p, 0, remoteAddress) >= 0){
		m_totalTx += p->GetSize();
		++m_seq;
		// ++m_trainParam;
		 
		NS_LOG_INFO("Worker " << m_workerId << " send packet " << m_seq << " to address " 
		<< Ipv4Address::ConvertFrom(m_peerAddress)
		<< " jobID: " <<m_jobId
		<< " Time:" << Simulator::Now().GetSeconds()
		<< " param "<<m_trainParam<<" epoch " << m_epochId);
		

	}else{
		NS_LOG_INFO("Worker " << m_workerId << " send packet " << m_seq << " to address " << Ipv4Address::ConvertFrom(m_peerAddress) << " port " << m_peerPort << " failed");
	}
...
InaWorker::HandleRead(Ptr<Socket> socket){
	...
	while((packet = socket->RecvFrom(from))){
		...
		if(packet->GetSize() > 0){
			...
			uint32_t packetType = inaH.GetPacketType();

			 

			// NS_ASSERT((jobId == m_jobId) && (epochId == m_epochId) && (workerId == m_workerId));
			if( m_workerId == 36 && (seq+1) == m_count  ){
			NS_LOG_INFO("Worker " << m_workerId << " receive packet " << seq << " epoch " << epochId <<" jobID: " <<jobId
								  << " Time:" << Simulator::Now().GetSeconds() << " Sequence:" << seq << " Param:" << param <<" packetType: "<< packetType);
			}
			if(jobId == m_jobId){
				CheckAndUpdate(seq);
			}
