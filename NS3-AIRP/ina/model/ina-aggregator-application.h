...
class InaJobStatus{
	public:
		...
		unordered_map<uint32_t, InaWorkerStatus> m_workerStatusMap; //denote the current worker-info belong to this job
...
class InaAggregator : public Application
{
	public:
		...
		void GetThroughput();
		void AddJob(uint32_t jobId, uint32_t workerNum, uint32_t head, uint32_t tail, map<uint32_t, ns3::Address> workerAddr, vector<uint32_t> workerIdList);
	...
	private:
		...
		*/
		...
		void CheckIfCanAgg(uint32_t jobId, uint32_t seqId);
		bool CheckJobDataReady(uint32_t jobId);
		int FindAggALU(uint32_t jobId);
		void DoAggregation(uint32_t aggAluId, uint32_t jobId, uint32_t seqId);
